...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    b.legalNeighbors.foldLeft(Stream[(Block, List[Move])]())(
        (s: Stream[(Block, List[Move])], neighbor: (Block, Move)) => 
          (neighbor._1, neighbor._2 :: history) #:: s)
  }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] = 
    from(Stream((Block(startPos, startPos), List[Move]())), Set[Block]())
  ...
  lazy val solution: List[Move] = 
    pathsToGoal.sortBy(x => x._2.length).toList(0)._2
}

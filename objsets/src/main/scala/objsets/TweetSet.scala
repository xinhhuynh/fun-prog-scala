...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet =
    //(left.union(right)).union(that) incl elem  <-- SLOW
...
object GoogleVsApple {
  ...
  def matchesList(keywords: List[String])(tweet: Tweet) : Boolean =
    keywords.exists(kw => tweet.text.contains(kw))
  
  lazy val allTweets: TweetSet = TweetReader.allTweets
  
  lazy val googleTweets: TweetSet = allTweets.filter(matchesList(google))
  lazy val appleTweets: TweetSet = allTweets.filter(matchesList(apple))
